...
- **최소 8명 최대 12명 (관전자는 해당 플레이어 수에 적용되지 않음.).**

- **3200 x 3200 맵에서 진행.**

- 네더, 오버월드 비율 1대1 (오버월드 1500, 1500에서 포탈 열면 네더 1500, 1500에서 나와짐).

- 엔드보더 500x500, 드래곤을 잡아도 엔드 시티 포탈은 미활성, 재소환 불가.
...
- 드래곤 알이 인벤토리에 있을시 상시 신속 1 + 성급함 1.

## 타겟 시스템

- 타겟 이외의 플레이어는 때리지 못함. (이 설정은 명령어로 변경 가능. `/chasingtails settings`)

    - 타겟이 헌터를 공격할 수 있음. 하지만 넉백만 가능하며 대미지 불가. 투사체로도 피해 가능.

    - 헌터의 노예는 대미지 및 사망까지 가능할 수 있음.

    - 타겟이 헌터를 살해까지 가능하게 하려면 명령어로 설정 변경 필요.
...
- 꼬리 잡기 색상 순서는 빨→주→노→초→파→남→보→검→빨 (인원에 따라 추가 색이 포함되어 약간 달라질 수 있음.)

## 사망 시스템

- 자연사 했을때: 죽은 자리에서 2분간 움직일 수 없다가 불사의 토템 효과와 함께 리스폰. (아이템 유지)

- 엔드에서 자연사하면 엔드 스폰지점(흑요석 스폰)에서 리스폰. (엔드 공허 무한 죽음 방지)

- 잡혀서 죽었을때: 아이템이 유지, 죽은 자리에서 리스폰되며 처치한 플레이어의 노예가 됨.
...
**게임 진행 도중에 플레이어가 접속을 종료하는 일이 없도록 해주세요. 게임 진행 상태를 저장하는 경우, 다른 명령어를 입력 할 필요 없이 `/stop` 명령만 입력하면 됩니다.**